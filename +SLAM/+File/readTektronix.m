% readTektronix Read Tektronix signal
%
% This function reads signal data from the binary files created by
% Tektronix digitizers.
%    [signal,time]=readTektronix(source);
% The input "source" indicates the file to be read.  When no file is
% specified, the user is prompted to interactively select the file.  Both
% the ISF and WFM binary formats are supported.
%
% The output "signal" contains voltages recorded in the digitizer file,
% which are acquired a locations stored in output "time"; both outputs are
% column arrays.  Calling the function with no output:
%    readTektronix(source);
% automatically plots the signal in a new figure window.
%
% See also SLAM.File
%
function varargout=readTektronix(file)

% manage input
if (nargin() < 1) || isempty(file)
    [file,location]=uigetfile('*.isf;*.ISF;*.wfm;*.WFM',...
        'Choose Tektronix binary file (*.isf, *.wfm)');
    if isnumeric(file)
        varargout{1}=[];
        varargout{2}=[];
        return
...
switch lower(ext)
    case '.isf'
        info=struct();
        [signal,time,info]=read_tektronixISF(file);
    case '.wfm'
        [signal,time,info]=read_tektronixWFM(file);
    otherwise
        error('ERROR: invalid Tektronix binary file');
end
...
%% ISF files
function [y,x,info]=read_tektronixISF(file)
...
% read header
info=dir(file);
bytes=min(info.bytes,1e6);
header=repmat(' ',[1 bytes]);
for n=1:bytes
    header(n)=fscanf(fid,'%1c',1);
    if endsWith(header(1:n),':CURVE')
        header=extractBefore(header,':CURVE');
        break
    end
end
...
while ~isempty(header)
    entry=extractBefore(header,';');
    while contains(entry,':')
        entry=extractAfter(entry,':');
    end
    name=upper(extractBefore(entry,' '));
    value=extractAfter(entry,' ');
    number=sscanf(value,'%g',1);
    if startsWith(name,'BIT_N')
        info.bits=number;
    elseif startsWith(name,'BYT_O')
        info.order=value;
    elseif startsWith(name,'NR_P')
        info.points=number;
    elseif startsWith(name,'XZE')
        info.x0=number;
    elseif startsWith(name,'XIN')
        info.dx=number;
    elseif startsWith(name,'PT_OFF')
        info.pt=number;
    elseif startsWith(name,'YOF')
        info.y0=number;
    elseif startsWith(name,'YMU')
        info.scale=number;
    elseif startsWith(name,'YZE')
        info.yz=number;
    end
    header=extractAfter(header,';');
end
...
temp=fscanf(fid,'%1c',1);
width=sscanf(temp,'%d',1);
[~]=fscanf(fid,'%1c',width);

% read data
switch upper(info.order)
    case 'LSB'
...
precision=sprintf('int%d',info.bits);
skip=0;
y=fread(fid,info.points,precision,skip,machineformat);
y=info.scale*(y-info.y0)+info.yz;

x=((0:info.points-1)-info.pt)*info.dx+info.x0;
x=x(:);
