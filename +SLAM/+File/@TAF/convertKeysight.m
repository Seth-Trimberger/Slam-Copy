% convertKeysight Convert Keysight binary file(s) to TAF
%
% This *static* method converts a Keysight binary file to *.taf file.
...
% extension, e.g. 'mydata.h5' is converted to 'mydata.taf'
%
...
% select one or more files.  Passing wildcard strings, such as '*.h5', in
% the "file" argument also launches multiple conversions.
% 
% See also TAF, File.readText
%
function convertKeysight(file,format)

% manage input
Narg=nargin();
if (Narg < 1) || isempty(file)
    file={'*.h5;*.H5' 'Keysight binary files'};
end
...
for n=1:numel(file)
    ...
    try
        [data,time]=SLAM.File.readKeysight(file{n});
