% Faddeva Evaluate Faddeva function
%
% This function evaluates the complex error function, also known as the
% Faddeeva function.
%    out=Faddeeva(in);
% A numerical array "in" passed to this function returns a numeric array
% "out" of the size and shape.  Note that either/both arrays may be complex
% valued.
%
% Note: this function is based on the MATLAB Central submission "fadf.m" by
% Sanjar Abrarov.  Underlying techncial details are displayed when the
% function is called with no input.
%    Faddeeva();
%
% See also SLAM.Math
%
function out=Faddeeva(in)

if (nargin() == 0) || isempty(in)
    location=fileparts(mfilename('fullpath'));
    file=fullfile(location,'Faddeeva','readme.txt');
    type(file);
    return    
end
